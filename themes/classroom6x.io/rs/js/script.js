...
function requestFullScreen(element) {
  ...
  if (requestMethod) {
    // Native full screen. - tự nhiên trên toàn màn hình
    requestMethod.call(element);
  } else if (typeof window.ActiveXObject !== "undefined") {
    // Older IE. - trình duyệt IE cũ
...
function cancelFullScreen() {
  ...
  //theater mode
  $(".header-game").removeClass(
    "force_full_screen header_game_enable_half_full_screen"
  );
  $("#iframehtml5").removeClass("force_half_full_screen");
...
function theaterMode() {
  ...
  if (iframe.classList.contains("force_half_full_screen")) {
    ...
    document
      .querySelector(".header-game")
      .classList.remove("header_game_enable_half_full_screen");
    return;
  }
  let above = 0;
  let left = 0;
  ...
  document
    .querySelector(".header-game")
    .classList.add("header_game_enable_half_full_screen");
}
...
document.addEventListener("DOMContentLoaded", function (event) {
  function get_search_data() {
    for (let a of $("div.card-masonry .item")) {
      ...
      let aTag = a.href;
      aTag =
        '<a class="games-show-item" href="' + aTag + '" title="' + name + '">';
      aTag +=
        '<img class="games-show-img" src="' +
        img +
        '" width="45" height="35" alt="' +
        name +
        '" title="' +
        name +
        '">';
